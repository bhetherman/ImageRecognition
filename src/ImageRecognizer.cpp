...
#include <ImageRecognizer/ImageRecognizer.h>
...
ImageRecognizer::ImageRecognizer() : pnh("~")
{
  ...
  ssSavedDataDirPath << ros::package::getPath("ImageRecognizer") << "/data/saveddata";
  stringstream ssImagesDirPath;
  ssImagesDirPath << ros::package::getPath("ImageRecognizer") << "/data/images";
  stringstream ssTestImagesDirPath;
  ssTestImagesDirPath << ros::package::getPath("ImageRecognizer") << "/data/testimages";
  stringstream ssNewImagesDirPath;
  ssNewImagesDirPath << ros::package::getPath("ImageRecognizer") << "/data/newimages/";
...
void ImageRecognizer::loadImageRecognizer()
{
  ...
  vocabFileNode = fs["vocabulary"];
  read( vocabFileNode, siftVocabulary);
  fs.release();
...
/*!
 * \brief Calculates and saves the feature vectors of the saved images
 */
...
void ImageRecognizer::detectExtractComputeFeatures(Mat colorInput, Mat &response_hist)
{
  ...
  siftDetector->detect(inputGS, keypoints);//detect feature points
...
/*!
 * \brief Normalizes the feature vector base on the number of pixels
 *
 * Normalizes the feature vector base on the number of pixels. ((x,y)/numPixels)
 *
 * \param featureVector A pointer to a matrix containing the feature values at each pixel
 * \param numPixels The number of pixels in the image from which featureVector was calculated
 * \return None
 */
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "ImageRecognizer");

    ImageRecognizer a;
    //a.loadImageRecognizer();
    a.trainImageRecognizer();
    a.testImageRecognizer();
